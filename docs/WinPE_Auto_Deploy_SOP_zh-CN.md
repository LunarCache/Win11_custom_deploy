...
适用版本：当前仓库主线实现，校验日期为 2026-04-24
...
该脚本支持 `-TargetDisk auto` 或显式磁盘号；`auto` 当前会解析为目标机上的磁盘 `0`。脚本还支持 `-DriversDirectory`，会在构建阶段把驱动目录复制进 `boot.wim` 根目录下的 `X:\drivers-payload`，部署时再离线注入到目标 Windows Driver Store。
...
`templates/deploy.cmd`：WinPE 内的主部署入口，负责发现介质、动态生成分区脚本、应用系统、写入引导、注入文件、复制载荷和保存日志。

`templates/unattend.xml`：配置 OOBE 跳过；隐藏网络设置和隐私设置页面；账户创建页面保持可见；不设置语言、区域、产品密钥或其他 OOBE 应答。

`templates/SetupComplete.cmd`：在系统部署后启用 WinRE，并注册首登自动化入口。

`templates/register-firstboot.ps1`：向 `HKLM\...\Run` 注册 `CodexFirstBoot`，并在检测到 Docker Desktop 时注册 `DockerDesktopAutoStart`。
...
如需指定固定目标盘，可把 `-TargetDisk auto` 改为 `-TargetDisk 0`、`-TargetDisk 1` 等明确磁盘号。`auto` 当前等价于部署时选择磁盘 `0`，并不会根据容量或总线类型自动推断。

如需在部署时注入离线驱动，可在构建阶段增加：
...
驱动目录必须在构建阶段提供；部署运行时不会从 USB 或 ISO 的 `payload\drivers` 目录自动发现驱动。
...
找到唯一镜像源后，脚本会动态生成 DiskPart 脚本并清空目标磁盘。默认分区布局如下：

1. EFI 分区，100 MB，盘符 `S:` (FAT32, label "System")
2. MSR 分区，16 MB
3. Windows 主分区，盘符 `W:` (NTFS, 大小可配置)
4. Recovery 分区，约 1024 MB，盘符 `R:` (NTFS, label "Recovery")

默认情况下，Windows 分区使用除 Recovery 之外的剩余空间。若指定了 `-WindowsPartitionSizeGB` 且同时指定 `-CreateDataPartition`，则会在 Windows 和 Recovery 之间增加 Data 分区，并使用剩余空间；若只指定固定 Windows 大小但不创建 Data 分区，Recovery 会按 `-RecoverySizeMB` 显式创建，磁盘上可能留下未分配空间。

随后脚本会执行以下动作：

1. `DISM /Apply-Image` 把指定索引映像应用到 `W:\`
2. `BCDBoot W:\Windows /s S: /f UEFI` 写入 UEFI 启动文件
3. 若 `X:\drivers-payload` 存在，则执行 `DISM /Image:W:\ /Add-Driver /Driver:X:\drivers-payload /Recurse`
4. 把 `unattend.xml` 复制到 `W:\Windows\Panther\unattend.xml`
5. 使用 `W:\Windows\System32\reagentc.exe /Setreimage` 设置 WinRE 路径
6. 把 `firstboot.ps1`、`firstboot-launcher.vbs`、`register-firstboot.ps1` 和 `SetupComplete.cmd` 注入目标系统
7. 若源介质存在 `\payload\docker-images`，则复制到 `W:\Payload\DockerImages`
8. 保存日志并自动关机，下一次开机从目标磁盘进入 Windows OOBE

注意：驱动注入、`unattend.xml`、WinRE 配置和首登资源注入都属于“有警告但不中断”的步骤。这些步骤失败会写入 warning，但在镜像应用和 UEFI 启动文件写入已经成功的前提下，不会阻止部署收尾。

## 10. 首次开机与首登阶段

系统首次进入已部署的 Windows 后，`SetupComplete.cmd` 会优先执行 `reagentc /enable`，然后调用 `register-firstboot.ps1` 注册 `CodexFirstBoot`。
...
5. `register-firstboot.ps1` 若在 SYSTEM 注册阶段发现 Docker Desktop，会写入 `HKLM\...\Run\DockerDesktopAutoStart`，为后续用户登录建立自启动。
6. `firstboot.ps1` 触发 `docker desktop start`，失败时回退启动 `Docker Desktop.exe`，然后等待 `Docker Desktop` 进程出现。
7. 之后通过多轮 `docker info` 轮询确认 daemon 已就绪。
8. Docker 就绪后，按目录名前缀顺序遍历 `C:\Payload\DockerImages\NN-name\`；每个服务目录内如存在 `load_images.bat` 则先执行，如存在 `install_service.bat` 则随后执行。
9. `load_images.bat` 和 `install_service.bat` 以可见 `cmd.exe` 窗口执行；窗口内容和成功后是否自动关闭由载荷批处理自身控制，核心脚本只等待进程退出并记录退出码。
10. 对于名称匹配 `*win11-install` 的服务，脚本会解析 `C:\CloudPrimeAppstore\docker-compose.yml` 中的端口和环境变量（`PANEL_USERNAME`、`PANEL_PASSWORD`、`PANEL_ENTRANCE`），并在成功后显示凭据窗口；解析失败或文件不存在时使用内置默认值。
11. 对于名称匹配 `*CIKE-install` 的服务，在成功后会显示 CIKE 部署成功信息窗口。
12. 任一载荷返回非零时，本次首登流程不写完成标记，等待下次登录继续重试。
13. 全部成功后，写入 `done.tag` 并移除 Run 项。
...
目标机成功自动分区并应用系统；Windows 分区位于 `W:`；EFI 分区位于 `S:`；部署后 WinPE 自动关机；再次开机后进入目标磁盘上的 Windows OOBE。

### 11.4 系统验收

Windows 首次进入 OOBE 时无线网络设置页面被隐藏，其余流程保持 Windows 标准行为；完成首次登录后，若存在载荷则能够进入 Docker 启动与载荷执行流程。
...
处理：检查目标磁盘健康、只读状态和容量；检查 `deploy.cmd` 中的分区逻辑是否被改动；检查 `install.wim` 是否损坏；检查目标机是否以 UEFI 模式启动。
...
1. 目标磁盘选择策略
2. 分区脚本（`:generate_diskpart_script` 子程序）
3. 镜像源发现机制
