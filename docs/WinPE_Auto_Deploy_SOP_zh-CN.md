...
首登载荷执行当前只按固定文件名识别：`load_images.bat` 和 `install_appstore.bat`。若需要新增载荷类型，必须修改模板脚本并重新构建 WinPE。
...
`templates/firstboot.ps1`：首次登录后等待 Docker Desktop 就绪，执行 `load_images.bat` 与 `install_appstore.bat`，全部成功后写入完成标记并删除 Run 项。
...
若需要导入 Docker 镜像或安装应用，必须准备好 `DockerImagesDirectory` 目录，至少确认以下文件是否存在并可执行：`load_images.bat`、`install_appstore.bat`、相关 `.tar` 镜像包以及脚本依赖文件。
...
8. Docker 就绪后，如存在 `load_images.bat` 则先执行；如存在 `install_appstore.bat` 则随后执行。
9. 任一载荷返回非零时，本次首登流程不写完成标记，等待下次登录继续重试。
10. 全部成功后，若 `install_appstore.bat` 成功，还会弹出 1Panel 凭据窗口，然后写入 `done.tag` 并移除 Run 项。
...
现象：`load_images.bat` 或 `install_appstore.bat` 返回非零，系统重复重试。
