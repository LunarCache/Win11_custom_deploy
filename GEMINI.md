...
- `Build-WinPEAutoDeploy.ps1`: Prepares the WinPE work directory. It injects the automation scripts into `boot.wim`, renders configuration tokens (Target Disk, WIM Index), and injects the `WinPE-WinReCfg` optional component to provide `reagentc` support.
...
- `deploy.cmd`: The main automation engine. Performs source discovery, disk partitioning via `diskpart-uefi.txt`, image application (DISM), and boot configuration (BCDBoot).
